...
use module_config
implicit none
    character(len=*), parameter :: PROCEDURE_NAME="test_enslocal"
...
!!    ld%ishorizon=.false.
    ...
    call system("cp  "//trim(ncfile)//" "//trim("enslocal_stdv.nc"))
    call modelstate_output(md, "enslocal_stdv.nc", stdv)
    ...
    do i=1, nvar
        do j=1, npts
            idx=md%vars(i)%ibgn-1+(ref_yid(j)-1)*80+ref_xid(j)
